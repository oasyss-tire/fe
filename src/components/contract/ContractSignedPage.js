...
const ContractSignedPage = () => {
  ...
  useEffect(() => {
    const fetchContractInfo = async () => {
      ...
      try {
        setLoading(true);
        const response = await fetch(`http://localhost:8080/api/signature/signed-contract?token=${token}`);
        ...
        if (!response.ok) {
          ...
          if (
            ...
            throw new Error('이 링크는 더 이상 유효하지 않습니다.\n최신 이메일에서 받은 링크를 사용해주세요.');
          }
          
          throw new Error(errorData.message || '서명된 계약 정보를 불러오는데 실패했습니다.');
        }
        
        const data = await response.json();
        if (data.success) {
          console.log('계약 정보 응답:', data);
          console.log('참여자 상태:', data.participant?.statusCode, data.participant?.statusName);
          console.log('상태 코드 ID:', data.participant?.statusCode?.codeId || data.participant?.statusCodeId);
          setContractInfo(data);
        } else {
          throw new Error(data.message || '서명된 계약 정보를 불러오는데 실패했습니다.');
        }
      } catch (err) {
        console.error('계약 정보 조회 오류:', err);
        setError(err.message || '계약 정보를 불러오는데 실패했습니다. 다시 시도해주세요.');
      } finally {
        setLoading(false);
      }
    };
    ...
    try {
      ...
      if (response.ok) {
        setResignRequestResult({
          success: true,
          message: data.message || '재서명 요청이 성공적으로 등록되었습니다. 관리자 승인 후 재서명이 가능합니다.'
        });
      } else {
        throw new Error(data.message || '재서명 요청 처리 중 오류가 발생했습니다.');
      }
    } catch (error) {
      console.error('재서명 요청 오류:', error);
      setResignRequestResult({
        success: false,
        message: error.message || '재서명 요청 중 오류가 발생했습니다. 다시 시도해주세요.'
      });
    } finally {
  ...
  const handleDownloadPdf = async (pdfId) => {
    try {
      ...
      window.open(`http://localhost:8080/api/contract-pdf/download-signed-pdf/${pdfId}`, '_blank');
    } catch (error) {
  ...
  const handleDownloadAllPdfs = async () => {
    try {
      if (!contractInfo || !contractInfo.participant || !contractInfo.participant.id) {
  ...
  const isParticipantStatus = (statusToCheck) => {
    ...
    const statusCode = 
      ...
      (typeof contractInfo.participant.statusCode === 'string' ? contractInfo.participant.statusCode : null);
    
    // 상태 이름을 통한 확인 (상태 코드가 없는 경우)
    const matchByName = 
      statusToCheck === "008001_0003" && contractInfo.participant.statusName === "서명 대기" ||
      statusToCheck === "008001_0006" && contractInfo.participant.statusName === "재서명 요청";
  ...
  if (loading) {
    return (
      <Box sx={{ 
        ...
        <Typography variant="h6">서명된 계약 정보를 불러오는 중입니다...</Typography>
      </Box>
    );
  }
  ...
  if (error) {
    // 토큰 만료 오류인지 확인
    const isTokenExpired = error.includes('이 링크는 더 이상 유효하지 않습니다');
    ...
    return (
      <Box sx={{ 
        ...
        >
          <Typography variant="h5" color={isTokenExpired ? "warning.main" : "error"} gutterBottom>
          ...
          {isTokenExpired ? (
            <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
              ...
              <Box sx={{ display: 'flex', flexDirection: 'row', gap: 2, justifyContent: 'center' }}>
                ...
                >
                  <img src="/images/google_logo.png" alt="Gmail" style={{ width: '35px', height: '35px', objectFit: 'contain' }} />
                </Button>
                <Button
                  variant="outlined"
                  size="small"
                  onClick={() => window.open('https://mail.naver.com', '_blank')}
                  sx={{ 
                    borderRadius: '50%',
                    minWidth: '48px',
                    width: '48px',
                ...
                >
                  <img src="/images/naver_logo.png" alt="네이버" style={{ width: '35px', height: '35px', objectFit: 'contain' }} />
                </Button>
                <Button
                  variant="outlined"
                  size="small"
                ...
                >
                  <img src="/images/daum_logo.png" alt="다음" style={{ width: '35px', height: '35px', objectFit: 'contain' }} />
                </Button>
              </Box>
            </Box>
          ) : (
  ...
  return (
    <Box sx={{ 
      ...
      {/* 서명 완료 상태 카드 */}
      <Paper elevation={0} sx={{ p: 4, mb: 3, borderRadius: 2, border: '1px solid #EEEEEE' }}>
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={6}>
            ...
            <Typography variant="body1">
              {isParticipantStatus("008001_0003")
                ? (participant.resignApprovedAt ? formatDate(participant.resignApprovedAt) : "-")
          ...
          <Grid item xs={12} md={6}>
            ...
            <Chip
              ...
              sx={{ 
                fontWeight: 500,
                backgroundColor: isParticipantStatus("008001_0003") ? '#FF9800' : '#3182F6',
                color: 'white'
              }}
            />
          </Grid>
        ...
        <Box sx={{ mt: 3, display: 'flex', justifyContent: 'flex-end' }}>
          {isParticipantStatus("008001_0003") ? (
            ...
            <Button
              variant="contained"
              color="warning"
              onClick={() => window.location.href = `/contract-sign/${contract.id}/participant/${participant.id}`}
              sx={{
                borderRadius: '4px',
                fontSize: '0.75rem',
                height: '32px',
      ...
      {/* 계약 문서 카드 */}
      <Paper elevation={0} sx={{ p: 4, borderRadius: 2, border: '1px solid #EEEEEE' }}>
          ...
          <Paper elevation={1} sx={{ p: 3, mt: 3, borderRadius: '8px' }}>
            <Box sx={{ display: 'flex', flexDirection: 'column', alignItems: 'center' }}>
              ...
              {!isParticipantStatus("008001_0003") ? (
                ...
                >
                  * PDF 파일은 암호로 보호되어 있으며, 암호는 이메일로 전송되었습니다.
                </Typography>
                </>
              ) : (
                <Typography 
                ...
                >
                  재서명이 필요한 문서입니다. 서명을 완료해야 다운로드할 수 있습니다.
                </Typography>
              )}
            </Box>
          </Paper>
        ) : (
          <Paper elevation={1} sx={{ p: 3, mt: 3, borderRadius: '8px', textAlign: 'center' }}>
          ...
          {!resignRequestResult ? (
            <>
              <Typography variant="body2" sx={{ mb: 3, color: '#505050' }}>
                이미 서명한 계약서에 대해 재서명을 요청합니다.
                재서명 요청 시 관리자 승인 후 새로운 서명 링크가 발송됩니다.
              </Typography>
              <Typography variant="body2" sx={{ mb: 1, color: '#505050', fontWeight: 500 }}>
                재서명 요청 사유
              </Typography>
              <TextField
                autoFocus
